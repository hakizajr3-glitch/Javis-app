...
import { Plus, Folder, FolderOpen, Paperclip, Brain, Clock, User, Settings, Loader2, Zap, Layers, ExternalLink, Sparkles, X } from "lucide-react";
import { SystemModal } from './SystemModal';
import { SystemConfigModal } from './SystemConfigModal';
import { conversationManager } from '../conversationManager';
import { jarvisRuntime } from '../jarvisConversationRuntime';
...
const ParticleSphere = ({ state, audioLevel = 0 }: { state: string; audioLevel?: number }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const animationRef = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      sortedParticles.forEach((p: any, index: number) => {
...
export const CenterHubExact = () => {
  ...
  useEffect(() => {
    conversationManager.setCallbacks({
      ...
      onTranscript: (_text: string, isFinal: boolean) => { if (isFinal) setCurrentTranscript(_text); },
      onError: (msg: string, _recoverable: boolean) => { setErrorMessage(msg); },
    });
  }, []);
  ...
  const handleInitialize = useCallback(async () => {
    if (aiActive) {
      ...
      try {
        ...
        if (!diag.allPassed) {
          // Show diagnostics but still try to initialize
          console.log('[CenterHub] Diagnostics:\n' + diag.summary);
        }
      } catch (e) {
        console.warn('[CenterHub] Diagnostics failed:', e);
      }

      // Initialize through ConversationManager (validates API key, Gemini, mic)
        ...
        if (SpeechRecognition) {
          ...
          recognition.onresult = async (event: any) => {
            ...
            if (text) {
              ...
              if (final.trim()) {
                // Route through JARVIS Conversation Runtime — parses intent,
                // executes real commands (shell, file, mouse, etc.) via Tauri,
                // and falls through to the AI for general conversation.
                const response = await jarvisRuntime.processInput(final.trim(), true);
                if (response === 'SWITCH_TO_HARNESS') {
                  // Navigate to Harness Control Center
                  (window as any).__jarvisNavigate?.('control-ui');
                }
              }
            }
          };
    ...
    <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex flex-col justify-center mr-4">
          <div className="bg-stonic-surface/80 backdrop-blur-sm border border-stonic-b1/50 rounded-[28px] px-3 py-5 flex flex-col items-center gap-4">
            <button
              ...
              className="w-14 h-14 rounded-2xl bg-stonic-card/50 border border-stonic-primary/60 flex flex-col items-center justify-center gap-1 shadow-[0_0_20px_rgba(0,216,238,0.25)] hover:shadow-[0_0_25px_rgba(0,216,238,0.4)] transition-all cursor-pointer"
              title="Import files for multimodal input"
            >
            ...
            <button
              ...
              className="w-14 h-14 rounded-2xl bg-stonic-card/30 border border-stonic-b1/30 flex flex-col items-center justify-center gap-1 hover:border-stonic-primary/40 hover:bg-stonic-card/50 transition-all cursor-pointer"
              title="Open desktop control panel"
            >
              <Folder size={18} className="text-stonic-textDim hover:text-stonic-primary transition-colors" />
              <span className="text-[6px] font-bold tracking-wider text-stonic-textDim hover:text-stonic-primary font-mono-tech text-center leading-tight transition-colors">
                STONIC_<br/>DSKTP
              </span>
            </button>
            ...
            <button
              ...
              className="w-14 h-14 rounded-2xl bg-stonic-card/30 border border-stonic-b1/30 flex flex-col items-center justify-center gap-1 hover:border-stonic-primary/40 hover:bg-stonic-card/50 transition-all cursor-pointer"
              title="Browse AI documents"
            >
              <FolderOpen size={18} className="text-stonic-textDim hover:text-stonic-primary transition-colors" />
              <span className="text-[6px] font-bold tracking-wider text-stonic-textDim hover:text-stonic-primary font-mono-tech text-center leading-tight transition-colors">
                MY AI<br/>DOCS
              </span>
            </button>
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  if (files && files.length > 0) {
                    ...
                    const fileMessage = `I've attached ${files.length} file${files.length > 1 ? 's' : ''}: ${fileNames.join(', ')}. Please analyze ${files.length > 1 ? 'them' : 'it'}.`;
                    setOrbState('processing');
                    try {
                      await jarvisRuntime.processInput(fileMessage, true);
                    } catch (err) {
                      setErrorMessage(`Failed to process file input: ${err}`);
                    }
                    setOrbState('idle');
                  }
      ...
      <div className="mt-4 bg-stonic-card border border-stonic-b1 rounded-xl p-3 relative overflow-hidden h-[180px]">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            onClick={() => setModalType('visual')}
            className="w-6 h-6 rounded-md bg-stonic-surface/50 border border-stonic-b1 flex items-center justify-center hover:bg-stonic-hover hover:border-stonic-primary/40 transition-all cursor-pointer"
            title="Open visual intelligence tools"
          >
            <ExternalLink size={10} className="text-stonic-textDim hover:text-stonic-primary transition-colors" />
          </button>
        </div>
      ...
      {modalType === 'settings' ? (
        ...
        <SystemModal
          ...
          onClose={() => setModalType(null)}
          type={modalType as 'memory' | 'history' | 'user' | 'desktop' | 'docs' | 'visual' | null}
        />
      )}
    </div>
  );
