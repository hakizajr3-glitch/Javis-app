...
import { ELEVENLABS_VOICES, type JarvisPersonaId, findJarvisPersona, getJarvisPersona, isElevenLabsPersona } from '../elevenLabsConfig';
...
const saveCfg = (partial: Partial<Record<string, string>>) => {
  const current = getSavedCfg();
  const next = { ...current, ...partial };
  localStorage.setItem('jarvis_config', JSON.stringify(next));
};

// Mirror the same Gemini key into the legacy standalone slot so the
// `aiManager` fast-path (which reads `localStorage.getItem('gemini_api_key')`
// without a JSON parse) sees the new value as well.
const saveStandaloneKey = (key: string) => {
  try { localStorage.setItem('gemini_api_key', key); } catch (_) {}
};

// Same dual-write pattern for ElevenLabs: any reader that does
// `localStorage.getItem('elevenlabs_api_key')` (without JSON parse) picks
// up the new value the moment the user clicks Save Configuration.
const saveStandaloneElevenlabsKey = (key: string) => {
  try { localStorage.setItem('elevenlabs_api_key', key); } catch (_) {}
};

// Mirror slot for the ElevenLabs voice id — same dual-write contract.
const saveStandaloneElevenlabsVoiceId = (voiceId: string) => {
...
export const SystemConfigModal: React.FC<SystemConfigModalProps> = ({ isOpen, onClose }) => {
  ...
  // DEFAULT_JARVIS_PERSONA). Subsequent updates from the restore-on-open
  // effect still work as before.
  const [voicePersona, setVoicePersona] = useState<JarvisPersonaId>(() => getJarvisPersona());
  const [selectedModel, setSelectedModel] = useState<string>(AIProviderConfig.fastModel);
  // ElevenLabs (streaming TTS). Optional — empty key triggers browser
  // SpeechSynthesis in voicePipelineFixed.ts / elevenLabsVoice.ts so JARVIS
  // still speaks when the user hasn't supplied one.
  ...
  // (Bella)". Setting to one of the catalog IDs collapses the dropdown to
  // that preset; setting to anything else switches the dropdown to Custom…
  const [elevenlabsVoiceId, setElevenlabsVoiceId] = useState('');
  const [elevenlabsVoicePreset, setElevenlabsVoicePreset] = useState<string>('default');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Restore the ElevenLabs key if the user previously saved one
      // (either through this field or via a legacy standalone slot).
      ...
      // Restore the ElevenLabs voice id (cfg takes precedence over the
      // standalone slot, same dual-write contract as the key).
      ...
      // If the resolved id matches a catalog preset, preselect it; otherwise
      // show Custom… so the user knows a non-preset id is in use.
  ...
  const selectPersona = (id: JarvisPersonaId) => {
    setVoicePersona(id);
  ...
  const handleSave = async () => {
    ...
    // 1. Renderer-local dual write so every reader path picks the new keys up:
    //    - jarvis_config.gemini_api_key  (RightPanel.getApiKey JSON path)
    //    - gemini_api_key                 (aiManager direct lookup)
    //    - jarvis_config.elevenlabs_api_key + elevenlabs_api_key mirror
    //                                    (any voice-pipeline fast path)
    //    - jarvis_config.elevenlabs_voice_id + elevenlabs_voice_id mirror
    saveCfg({
    ...
    // 2. Tauri-only: push to the in-process ConfigurationManager so the IPC
    //    singleton stays in sync with the renderer's localStorage copy.
    try {
      await ConfigurationManager.getInstance().save({
        gemini_api_key: trimmed,
        elevenlabs_api_key: elTrimmed,
        elevenlabs_voice_id: elVoiceTrimmed,
        voice_persona: persona,
      ...
      console.warn('[SystemConfigModal] ConfigurationManager.save failed:', e);
    }
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="p-4 rounded-xl bg-black/40 border border-white/10">
              ...
              <p className="text-[10px] text-white/40 mb-3">Picks the model the AI service uses for chat and voice replies. Fallbacks try the next model if the primary fails (e.g. quota).</p>
              <select
                value={selectedModel}
          ...
          <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex items-start gap-3 mb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[9px] text-white/55">{jarvisMaleEntry?.shortDescription}</div>
                    </div>
                  </div>
                  <p className="text-[9px] text-white/45 italic leading-relaxed line-clamp-4">
                    {jarvisMaleEntry?.description}
                  </p>
                </button>
              );
            })()}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div className="text-[8px] text-white/45">{findJarvisPersona('male')?.description}</div>
              </button>
              ...
              >
                ...
                <div className="text-[8px] text-white/45">{findJarvisPersona('female')?.description}</div>
              </button>
            </div>
          </div>

          {/* ---- Voice Synthesis (ElevenLabs) — only for ElevenLabs-backed personas ---- */}
          {isElevenLabsPersona(voicePersona) && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-white/80">
            ...
            <div className="p-4 rounded-xl bg-black/40 border border-white/10">
              ...
              <p className="text-[9px] text-white/30 mt-2">Set VITE_ELEVENLABS_API_KEY in <span className="text-white/50">jarvis-tauri/.env</span> for a build-time default. UI input overrides it at runtime.</p>
            </div>
          </div>
          )}

          {/* ---- Voice (ElevenLabs preset + voice ID) — only for ElevenLabs-backed personas ---- */}
          {isElevenLabsPersona(voicePersona) && (
          <>
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-white/80">
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      // Any direct typing overrides the preset to Custom…
