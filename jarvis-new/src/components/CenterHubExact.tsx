...
import { Plus, Folder, FolderOpen, Paperclip, Brain, Clock, User, Loader2, Zap, Layers, ExternalLink, Sparkles } from "lucide-react";
...
// ElevenLabs TTS configuration - set via environment variables.
// The renderer holds no secrets; production wiring reads keys from the main
// process via window.electronAPI.getConfig() (added in preload.ts). Empty
// string fallbacks here only determine whether the renderer-side
// `speakText` fast-path engages — an empty key forces it through
// window.electronAPI.synthesize() via IPC, matching source architecture.
