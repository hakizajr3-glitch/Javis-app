...
export class GeminiLiveSession {
  ...
  async start(apiKey, videoEl, callbacks = {}) {
    if (this.state !== 'idle') {
  ...
  connectWebSocket() {
    return new Promise((resolve, reject) => {
      ...
      this.ws.onclose = (ev) => {
        ...
        if (this.state !== 'idle') {
          this.setState('error');
          this.callbacks.onError?.(`Gemini Live connection closed (${ev.code})`);
        }
  ...
  async queuePcmAudio(pcm, sampleRate) {
