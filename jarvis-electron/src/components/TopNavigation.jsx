...
import { Cpu, FileText, CheckSquare, Users, Settings, Gauge } from 'lucide-react';
...
const tabs = [
  ...
  { id: 'control-ui', label: 'Control UI', icon: <Gauge size={16} /> },
];

export const TopNavigation = ({ activeTab, onTabChange }) => {
  return (
    <header className="h-11 bg-stonic-surface border-b border-stonic-b2 flex items-center justify-between px-6 shrink-0 relative z-50">
      ...
      <nav className="flex items-center gap-1">
        {tabs.map((tab) => (
          <button
            ...
            className={`
              relative px-4 py-2 rounded-lg flex items-center gap-2 font-rajdhani font-semibold text-sm tracking-wide
      ...
      <button className="p-2 rounded-lg text-stonic-textMuted hover:text-stonic-primary
